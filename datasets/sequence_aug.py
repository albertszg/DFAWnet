...
class wgn(object):#信噪比越大越好
    def __init__(self, snr=3,sigma=0.02):#0.01 MFPT 0.03 XJTU
        self.snr = 10**(snr/10.0)
        self.sigma = sigma

    def __call__(self, seq):
        xpower = np.sum(seq**2)/len(seq)#P signal
        npower = xpower/self.snr#P noise=P signal/ 10**(P signal/10)
        return seq + np.random.normal(loc=0, scale=self.sigma, size=seq.shape)*np.sqrt(npower)